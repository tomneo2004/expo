...
const loadAppFromUrl = async (url: string): bool => {
...
const App = () => {
  ...
  const tryLoadAppFromUrl = (url: string) => {
    (async () => {
      if (!await loadAppFromUrl(url)) {
        setLoading(false);
      }
    })();
  };
  ...
  const onPressGoToUrl = () => {
    tryLoadAppFromUrl(textInputUrl);
    setLoading(true);
  };
